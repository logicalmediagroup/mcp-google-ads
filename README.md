# Google Ads MCP - Logical Media Group Edition

![Google Ads MCP](bg.jpeg)

A production-ready tool that connects [Google Ads](https://ads.google.com/) with AI assistants (Claude, Cursor, LangGraph agents), allowing you to analyze advertising data through natural language conversations. This **custom edition for Logical Media Group** includes enterprise-grade features like Cloud Run deployment, service account authentication, and HTTP API endpoints for seamless integration with internal tools and agents.
...
This MCP server provides tools accessible through multiple interfaces:
- **🤖 AI Assistants** (Claude, Cursor) via MCP protocol
...
## 🏢 Logical Media Group Deployment Architecture

This custom version includes several enterprise enhancements:

### Cloud Run Deployment
- **Serverless**: Auto-scaling Cloud Run service handling HTTP requests
- **Authentication**: Google Cloud identity tokens for secure access
...
### Deployment Files
- **`Dockerfile`**: Multi-stage build with AMD64 architecture
- **`deploy.sh`**: Automated deployment script
- **`cloudbuild.yaml`**: CI/CD pipeline configuration
- **`simple_mcp_client.py`**: Production client example
...
### Option 1: Use the Deployed LMG Service (Recommended for LMG Team)
If you're part of the Logical Media Group team, you can directly use the deployed service:

1. **Get Service Account Key**: Contact IT for `[SERVICE_ACCOUNT_KEY_FILE]`
...
3. **Use the Client**:
   ```python
   from simple_mcp_client import GoogleAdsMCPClient
   
   client = GoogleAdsMCPClient(
       service_account_path="./[SERVICE_ACCOUNT_KEY_FILE]",
       mcp_server_url="https://[CLOUD_RUN_URL]"
   )
...
### Option 2: Local Development Setup (For Custom Development)
...
### Start Analyzing Your Advertising Data!

Now you can ask AI assistants questions about your Google Ads data! The tools can not only retrieve the data but also analyze it, explain trends, and create visualizations to help you understand your advertising performance better.

#### For Claude/Cursor Users (MCP Protocol)
...
| `execute_gaql_query`            | "Execute this query for account [CUSTOMER_ID]: SELECT campaign.name, metrics.clicks FROM campaign WHERE metrics.impressions > 1000" |
| `get_campaign_performance`      | "Show me the top 10 campaigns for account [CUSTOMER_ID] in the last 30 days, highlight any with ROAS below 2, and suggest optimization strategies." |
...
#### For LangGraph Agents (HTTP API)
Use the deployed Cloud Run service in your agents:
...
response = requests.post(
    'https://[CLOUD_RUN_URL]/mcp/get_campaign_performance',
    headers=headers,
    json={'customer_id': '[CUSTOMER_ID]', 'days': 30}
)

data = response.json()
print(data['data'])  # Campaign performance results
```
...
## 🔧 Technical Enhancements for LMG

This custom version includes several key technical improvements:
...
- **Google Cloud Identity Tokens**: Secure authentication for Cloud Run services
- **Domain-Wide Delegation**: Enterprise-grade access control

### Docker Containerization
Optimized for Cloud Run deployment:
- **Multi-architecture support**: AMD64 builds on Google's infrastructure
- **Security**: Non-root user, minimal attack surface
- **Performance**: Efficient layer caching and dependency management
...
{
  "success": true,
  "data": "Actual response data from Google Ads API"
}
```
...
This is a **custom enterprise version** for Logical Media Group. For the original open-source version, visit the [ixigo repository](https://github.com/ixigo/mcp-google-ads).

For LMG-specific issues or improvements:
- Contact the development team internally
- Submit pull requests to the LMG fork
- Coordinate with IT for deployment changes

---

## License

This project is based on the MIT-licensed [Google Ads MCP by ixigo](https://github.com/ixigo/mcp-google-ads). The LMG customizations maintain the same MIT License. See the [LICENSE](LICENSE) file for details.

---

## About This Implementation

**Customized for Logical Media Group** 
This enterprise version includes production-grade enhancements for internal use, including Cloud Run deployment, service account authentication, and HTTP API endpoints for seamless integration with LangGraph agents and other internal tools.

**Original Project by ixigo**  
Based on the open-source Google Ads MCP tool by ixigo, India's leading travel technology company. 

<img src="ixigo-logo.png" alt="ixigo logo" width="200px" />

For more information about the original project, visit [ixigo.com](https://www.ixigo.com).
